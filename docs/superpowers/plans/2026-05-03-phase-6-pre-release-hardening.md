...
- [ ] **Step 1: Confirm `main` is clean and Phase 5 committed**
...
- [ ] **Step 2: Bump version in `scripts/agent_forge/pyproject.toml` and `__init__.py` to `1.0.0`**
...
- [ ] **Step 4: Watch the integration matrix run**
...
- [ ] **Step 1: Wait for matrix completion** (~10-15 min)
...
- [ ] **Step 1:** spin a fresh container with Claude installed; follow `docs/install/claude-code.md` step-by-step
- [ ] **Step 2:** install all 4 plugins (`writing`, `prompts`, `msft-arch`, `pm`)
- [ ] **Step 3:** invoke one skill from each plugin; verify output is what the SKILL.md claims
- [ ] **Step 4:** run `agent-forge update --check`; verify reports "up-to-date"
- [ ] **Step 5:** record any docs gaps as issues with label `area/docs`
...
- [ ] **Step 1:** Run `pip-audit` on the agent-forge runtime dependencies
...
- [ ] **Step 2:** Same for tests dependencies
...
- [ ] **Step 1:** Confirm GitHub secret scanning is enabled
...
- [ ] **Step 2:** Spot-check committed files for accidental secrets
...
- [ ] **Step 1:** Verify every per-plugin `THIRD_PARTY_NOTICES.md` is accurate
...
- [ ] **Step 1:** Verify auto-generated artifacts haven't drifted from canonical
...
- [ ] **Step 1:** Final review of `tests/_legacy/kpmg_harness_reference.py` for any patterns worth porting
...
- [ ] **Step 3:** Verify no test references the legacy path
...
- [ ] **Step 2: Verify all sync invariants**
...
- [ ] **Step 3: Verify the release branch matches main + version bump**
...
- [ ] **Step 4: Open the release PR (release branch → main, tag-ready)**
...
- [ ] **Step 5: Wait for CI on the PR; require human review** (this is the final gate before tagging)

---

## Self-Review

- [ ] All Layer C integration tests passing on release/v1.0.0 ✓
- [ ] All ~150 skill evals within tolerance ✓
- [ ] Security review complete with zero open findings ✓
- [ ] tests/_legacy/ removed ✓
- [ ] Release notes drafted ✓
- [ ] PR open from release/v1.0.0 → main ✓
